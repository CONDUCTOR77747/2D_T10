...
from parse_excel import parse_interval
...
for i in range(len(shots)):
    ...
    if inspect_scans_flag:
        ...
        df_buffer[f"{time_intervals[i]}_x"] = A_QCM.x
        df_buffer[f"{time_intervals[i]}_y"] = A_QCM.y
        
        df_inspector = pd.concat([df_inspector, df_buffer], axis=1, ignore_index=False)
...
if inspect_scans_flag:
    ...
    for ti in time_intervals:
        ax_inspector.plot(df_inspector[f'{ti}_x'], df_inspector[f'{ti}_y'])
        ax_inspector.axvspan(parse_interval(ti)[0], parse_interval(ti)[1], facecolor='g', alpha=0.5)
        break
    plt.show()
