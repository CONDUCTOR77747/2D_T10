...
import os
from loadsig import WinRegKey, SignalError
import winreg 
import glob
...
def readItem(cfgParser, section, item):
    ...
    result = result.split(';')[0]   #result = str.replace(result, ";", "")
    return result
...
def readThomsonPath(configFileName): 
    rpccCfg = cfg.ConfigParser(comment_prefixes=('!', ';'), 
                               inline_comment_prefixes=(';',))
    rpccCfg.read(configFileName)
    result = readItem(rpccCfg, "Cache", "ThomsonPath")
    return result   
...
def findCacheFile(pathList, shot, signalName): 
    for path in pathList: 
        ...
        if os.path.exists(fname): return (True, fname)
    return (False, "")

def parseCacheFileName(fileName): 
    '''
    dev, shot, name = parseCacheFileName(cacheFile)   
    '''
...
def findRpccWrapCfg(): 
    ...
    raise SignalError("File rpccwrap.cfg not found")
...
def readMyBuffer(file): 
    ...
    if (L == ZIP_BUF_SIGNATURE)and(elem_size == 0): 
...
def loadCacheFile(fileName): 
    with open(fileName, "rb") as cache_file: 
        ...
        compression_info = readByte(cache_file)
        ...
        if compression_info == 0: 
            y_bytes = cache_file.read(data_length*4)
                    
        elif compression_info == 1: 
            compressed_size = readLongInt(cache_file)            
            readLongInt(cache_file)                     # uncompressed_size
            cache_file.read(4*extended_header_cnt - 8)  # reserved
...
def loadCachedSignal(shot, signalName): 
    ...
    if ok: 
        ...
        raise SignalError("Signal #%d %s not found in the cache" % (shot, signalName))
...
def loadCarpetFile(fileName): 
   with open(fileName, "rb") as carp_file: 
       ...
       readDouble(carp_file)  # min_v = readDouble(carp_file)
       readDouble(carp_file)  # max_v = readDouble(carp_file)     
       readDouble(carp_file)  # mask_level = readDouble(carp_file)     
       
       readMyStr(carp_file)   # title = readMyStr(carp_file)  

       readLongInt(carp_file) # mask_color = readLongInt(carp_file)  # color
       readByte(carp_file)    # show_cmap = readByte(carp_file)   # bool
       readLongInt(carp_file) # orig_y_len = readLongInt(carp_file)
...
if __name__ == '__main__':
    ...
    sig = loadCachedSignal(33706, 'ABOL8') 
    plt.plot(sig[0, :], sig[1, :])
    '''  
    '''
    import matplotlib.pyplot as plt
    fname = "H:\\cache\\CompassCacheNew\\cmps_6358_Mirnov_coil_B_theta_06_RAW.cache" 
    (xx, yy) = loadCacheFile(fname)   
    plt.plot(xx, yy)
    ''' 
